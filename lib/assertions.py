...
class Assertions:
    ...
    def assert_json_has_not_key(response: Response, name):
        ...
        assert name not in response_as_dict, f"Response JSON should not have '{name}', but it is present."
    ...
    def assert_code_status(response: Response, expected_status_code):
        assert response.status_code == expected_status_code, f"Unexpected status code. \
         Expected: '{expected_status_code}', Actual: '{response.status_code}'"

    @staticmethod
    def assert_expected_response_content(response: Response, expected_content):
        assert response.content.decode('utf-8') == expected_content, \
            f"Unexpectable response content: '{response.content}'"
